...
@tool
def compute_truncated_l1_distance(embeddings: Embeddings, text1: str, text2: str, min_max_first: str = "first", how_many: int = 1) -> float:
    """
    # GA Truncated L1 Distance

    Computes the truncated L1 distance between the embedding vectors of two texts, that is, the sum of absolute values of a given number of coordinate changes between the vectors. Computes the sum of changes for the biggest, smallest or first listed changes, depending on the parameter `min_max_first`.

    For example, compute_average_truncated_l1_distance(model, text1, text2, "min", 100) computes the sum of absolute values of the smallest 100 changes between the embedding vectors.
    ...
    if min_max_first == "first":
        return np.sum(distances[0:how_many])
